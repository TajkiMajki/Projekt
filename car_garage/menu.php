...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM shop WHERE code='" . $_GET["code"] . "'");
    ...
    case "insert":
		if(!empty($_SESSION["cart_item"])) {
                foreach($_SESSION["cart_item"] as $k => $v){
                    ...
                    $done1=$db_handle->insq("insert into orders (`email`,`ord_date`,`code`,`quantity`,`ord_time`,`total`, `order_id`) values ('$email1','$ord_date1','$code1','$quantity1','$ord_time1','$total', 'order_id')");    
                }
...
<body data-aos-easing="ease-out-back" data-aos-duration="1500" data-aos-delay="0">
    ...
    <div class="col-md-7" id="product-grid">
        <?php
	$product_array = $db_handle->runQuery("SELECT * FROM shop ORDER BY id asc");
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;"><a
                                href="menu.php?action=remove&code=<?php echo $item["code"]; ?>"><img
                                    src="img\icon-delete.png" alt="Remove Item" /></a></td>
...
?>
            ...
            <a type="button" onclick="myFunction()" id="btnorder"
                ...
                <p id="myP" style="visibility: hidden;font-family:'AbolitionW00-Regular';color: #d00000;float:left; text-decoration: none;
